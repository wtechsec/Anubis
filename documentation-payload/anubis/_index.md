...
- **RDP self-setup + access**: `rdp_ext` configures RDP on the agent's own Windows host in one command: enables RDP in the registry (`fDenyTSConnections=0`), changes the port to 6000 (avoids default 3389 IDS rules), adds an inbound firewall rule via `netsh`, restarts `SessionEnv` and `TermService` via ctypes SCM without spawning `sc.exe` (SessionEnv owns the RDP-Tcp listener and must be restarted to apply the port change), probes the target port, and returns xfreerdp/rdesktop commands. xfreerdp's native `/proxy:socks5://` eliminates the need for proxychains on the operator machine (T1021.001, T1090, T1562.004).
...
- `rdp_ext`: habilita RDP no host do agente (registry + porta 6000 + firewall + restart TermService via SCM), sobe SOCKS5 no Mythic e retorna comandos xfreerdp/rdesktop prontos — T1021.001, T1562.004
